...
| Safety check reports a real host binding such as `0.0.0.0:10531` | A manual or altered Compose configuration published the port. | Do not bypass the safety check. Inspect the separate sidecar project, remove the `ports:` mapping from that sidecar only, and redeploy it without touching n8n. |
